...
def show_text(msg, xx, yy, color, size, font):
...
randomGraphics = []
menuGraphics = []
player = screen.blit(idle_right[idle], (x, y))
        ...
        if not pause:
            ...
            if down:
                ...
                if y > floor + floorDifference:
                    ...
                    jumptime = time.time()
                if side:
                    player = screen.blit(jump_left[8], (x, y))
                else:
                    player = screen.blit(jump_right[8], (x, y))
                floorDifference = 10
            elif right:
                side = False
                if x < 675:
                    x += 8
                player = screen.blit(walk_right[walk], (x, y))
                floorDifference = 7
            elif left:
                side = True
                if x > 0:
                    x -= 8
                player = screen.blit(walk_left[walk], (x, y))
                floorDifference = 7
            else:
                if not attacking:
                    if side:
                        player = screen.blit(idle_left[idle], (x, y))
                    else:
                        player = screen.blit(idle_right[idle], (x, y))
                    floorDifference = 4
